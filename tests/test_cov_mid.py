...
class TestShellEnvCheck:
    def test_dangerous_env(self, tmp_path):
        ...
        r = _run(firm_shell_env_check(config_path=cfg))
...
class TestTokenSeparationCheck:
    def test_same_token(self, tmp_path):
        ...
        r = _run(firm_token_separation_check(config_path=cfg))
        assert isinstance(r, dict)
...
class TestRpcRateLimitCheck:
    def test_remote_no_rate_limit(self, tmp_path):
        ...
        r = _run(firm_rpc_rate_limit_check(config_path=cfg))
        assert isinstance(r, dict)
...
class TestSecurityScan:
    def test_scan_with_vulns(self, tmp_path):
        ...
        r = _run(firm_security_scan(target_path=str(tmp_path)))
        assert isinstance(r, dict)
        assert r.get("total_files_scanned", 0) >= 1

    def test_scan_nonexistent(self):
        from src.security_audit import firm_security_scan
        r = _run(firm_security_scan(target_path="/nonexistent/path"))
        assert isinstance(r, dict)
    ...
    def test_scan_depth_limit(self, tmp_path):
        ...
        r = _run(firm_security_scan(target_path=str(tmp_path), scan_depth=2))
        assert isinstance(r, dict)
...
class TestSandboxAudit:
    def test_sandbox_off(self, tmp_path):
        ...
        r = _run(firm_sandbox_audit(config_path=str(cfg)))
        assert isinstance(r, dict)
...
class TestSessionConfigCheck:
    def test_env_file(self, tmp_path):
        ...
        r = _run(firm_session_config_check(env_file_path=str(env_file)))
        assert isinstance(r, dict)
    ...
    def test_compose_file(self, tmp_path):
        ...
        r = _run(firm_session_config_check(compose_file_path=str(compose)))
        assert isinstance(r, dict)
...
class TestRateLimitCheck:
    def test_with_funnel(self, tmp_path):
        ...
        r = _run(firm_rate_limit_check(gateway_config_path=str(cfg)))
        assert isinstance(r, dict)
...
class TestSecretsLifecycleCheck:
    def test_inline_creds(self, tmp_path):
        ...
        r = _run(firm_secrets_lifecycle_check(config_path=cfg))
        assert isinstance(r, dict)
...
class TestChannelAuthCanonCheck:
    def test_remote_no_auth(self, tmp_path):
        ...
        r = _run(firm_channel_auth_canon_check(config_path=cfg))
        assert isinstance(r, dict)
...
class TestConfigPrototypeCheck:
    def test_prototype_pollution(self, tmp_path):
        ...
        r = _run(firm_config_prototype_check(config_path=cfg))
        assert isinstance(r, dict)
...
class TestGroupPolicyDefaultCheck:
    def test_missing_policy(self, tmp_path):
        ...
        r = _run(firm_group_policy_default_check(config_path=cfg))
        assert isinstance(r, dict)
...
class TestSecretsV2Audit:
    def test_hardcoded_keys(self, tmp_path):
        ...
        r = firm_secrets_v2_audit(config_path=cfg)
        assert isinstance(r, dict)
...
class TestAgentRoutingCheck:
    def test_circular_routing(self, tmp_path):
        ...
        r = firm_agent_routing_check(config_path=cfg)
        assert isinstance(r, dict)
...
class TestVoiceSecurityCheck:
    def test_dangerous_provider(self, tmp_path):
        ...
        r = firm_voice_security_check(config_path=cfg)
        assert isinstance(r, dict)
...
class TestTrustModelCheck:
    def test_shared_dm_scope(self, tmp_path):
        ...
        r = firm_trust_model_check(config_path=cfg)
        assert isinstance(r, dict)
...
class TestAutoupdateCheck:
    def test_risky_channel(self, tmp_path):
        ...
        r = firm_autoupdate_check(config_path=cfg)
        assert isinstance(r, dict)
...
class TestPluginSdkCheck:
    def test_dangerous_hooks(self, tmp_path):
        ...
        r = firm_plugin_sdk_check(config_path=cfg)
        assert isinstance(r, dict)
...
class TestContentBoundaryCheck:
    def test_missing_boundaries(self, tmp_path):
        ...
        cfg = _write_config(tmp_path, {})
        r = firm_content_boundary_check(config_path=cfg)
        assert isinstance(r, dict)
...
class TestSqliteVecCheck:
    def test_traversal_path(self, tmp_path):
        ...
        r = firm_sqlite_vec_check(config_path=cfg)
        assert isinstance(r, dict)
...
class TestN8nWorkflowExport:
    def test_basic_export(self):
        ...
        r = _run(firm_n8n_workflow_export(pipeline_name="test", steps=steps))
        assert isinstance(r, dict)
    ...
    def test_export_to_file(self, tmp_path):
        ...
        r = _run(firm_n8n_workflow_export(pipeline_name="test", steps=steps, output_path=out))
        assert isinstance(r, dict)
...
class TestN8nWorkflowImport:
    def test_valid_import(self, tmp_path):
        ...
        r = _run(firm_n8n_workflow_import(workflow_path=str(wf_path)))
        assert isinstance(r, dict)
        assert r.get("ok") is True
    ...
    def test_missing_fields_strict(self, tmp_path):
        ...
        r = _run(firm_n8n_workflow_import(workflow_path=str(wf_path), strict=True))
        assert isinstance(r, dict)
...
class TestI18nAudit:
    def test_json_locales(self, tmp_path):
        ...
        r = _run(firm_i18n_audit(project_path=str(tmp_path)))
        assert isinstance(r, dict)
        assert r.get("ok") is True
    ...
    def test_yaml_locales(self, tmp_path):
        ...
        r = _run(firm_i18n_audit(project_path=str(tmp_path), file_format="yaml"))
        assert isinstance(r, dict)
    ...
    def test_properties_format(self, tmp_path):
        ...
        r = _run(firm_i18n_audit(project_path=str(tmp_path), file_format="properties"))
        assert isinstance(r, dict)
...
class TestGatewayAuthCheckDeep:
    def test_funnel_no_password(self, tmp_path):
        ...
        r = _run(firm_gateway_auth_check(config_path=cfg))
        assert isinstance(r, dict)
...
class TestCredentialsCheckDeep:
    def test_stale_credentials(self, tmp_path):
        ...
        r = _run(firm_credentials_check(credentials_dir=str(creds_dir)))
        assert isinstance(r, dict)
...
class TestWebhookSigCheckDeep:
    def test_missing_secret(self, tmp_path):
        ...
        r = _run(firm_webhook_sig_check(config_path=cfg))
        assert isinstance(r, dict)
    ...
    def test_single_channel(self, tmp_path):
        ...
        r = _run(firm_webhook_sig_check(config_path=cfg, channel="slack"))
        assert isinstance(r, dict)
...
class TestLogConfigCheckDeep:
    def test_debug_level(self, tmp_path):
        ...
        r = _run(firm_log_config_check(config_path=cfg))
        assert isinstance(r, dict)
...
class TestWorkspaceIntegrityCheckDeep:
    def test_complete_workspace(self, tmp_path):
        ...
        r = _run(firm_workspace_integrity_check(workspace_dir=str(tmp_path)))
        assert isinstance(r, dict)
...
class TestProvenanceTracker:
    def test_append_and_verify(self):
        ...
        r2 = firm_provenance_tracker(action="verify")
        assert isinstance(r2, dict)

        r3 = firm_provenance_tracker(action="export")
        assert isinstance(r3, dict)

        r4 = firm_provenance_tracker(action="status")
...
class TestSecretsWorkflowCheck:
    def test_hardcoded_secrets(self, tmp_path):
        ...
        r = _run(firm_secrets_workflow_check(config_path=cfg))
        assert isinstance(r, dict)
...
class TestHttpHeadersCheckDeep:
    def test_public_bind(self, tmp_path):
        ...
        r = _run(firm_http_headers_check(config_path=cfg))
        assert isinstance(r, dict)
...
class TestNodesCommandsCheckDeep:
    def test_allow_commands_remote(self, tmp_path):
        ...
        r = _run(firm_nodes_commands_check(config_path=cfg))
        assert isinstance(r, dict)
...
class TestTrustedProxyCheckDeep:
    def test_empty_trusted_proxies(self, tmp_path):
        ...
        r = _run(firm_trusted_proxy_check(config_path=cfg))
        assert isinstance(r, dict)
...
class TestSessionDiskBudgetCheckDeep:
    def test_missing_budget(self, tmp_path):
        ...
        cfg = _write_config(tmp_path, {})
        r = _run(firm_session_disk_budget_check(config_path=cfg))
        assert isinstance(r, dict)
...
class TestDmAllowlistCheckDeep:
    def test_open_dm_policy(self, tmp_path):
        ...
        r = _run(firm_dm_allowlist_check(config_path=cfg))
        assert isinstance(r, dict)
...
class TestAgentTeamOrchestrate:
    def test_parallel_tasks(self):
        ...
        r = _run(firm_agent_team_orchestrate(tasks=tasks, objective="Research"))
        assert isinstance(r, dict)
        assert r.get("ok") is True
    ...
    def test_sequential_deps(self):
        ...
        r = _run(firm_agent_team_orchestrate(tasks=tasks, objective="Pipeline"))
        assert isinstance(r, dict)
    ...
    def test_cycle_detection(self):
        ...
        r = _run(firm_agent_team_orchestrate(tasks=tasks))
        assert isinstance(r, dict)
        assert r.get("ok") is False
    ...
    def test_vote_aggregation(self):
        ...
        r = _run(firm_agent_team_orchestrate(
...
class TestAgentTeamStatus:
    ...
    def test_nonexistent_id(self):
        from src.agent_orchestration import firm_agent_team_status
        r = _run(firm_agent_team_status(orchestration_id="nonexistent"))
        assert isinstance(r, dict)
        assert r.get("ok") is False
...
class TestObservabilityPipeline:
    def test_ingest_jsonl(self, tmp_path):
        ...
        r = _run(firm_observability_pipeline(jsonl_path=str(jsonl), db_path=db))
        assert isinstance(r, dict)
        assert r.get("ok") is True
    ...
    def test_wrong_extension(self, tmp_path):
        ...
        txt.write_text("{}\n")
        r = _run(firm_observability_pipeline(jsonl_path=str(txt)))
        assert isinstance(r, dict)
        assert r.get("ok") is False
    ...
    def test_duplicate_handling(self, tmp_path):
        ...
        r = _run(firm_observability_pipeline(jsonl_path=str(jsonl), db_path=str(tmp_path / "d.db")))
        assert isinstance(r, dict)
...
class TestSkillLazyLoader:
    def test_load_all(self, tmp_path):
        ...
        """))
        r = _run(firm_skill_lazy_loader(skills_dir=str(tmp_path / "skills")))
        assert isinstance(r, dict)
        assert r.get("ok") is True
    ...
    def test_refresh_cache(self, tmp_path):
        ...
        skill.mkdir(parents=True)
        (skill / "SKILL.md").write_text("---\nname: cached\n---\n# Cached\n")
        _run(firm_skill_lazy_loader(skills_dir=str(tmp_path / "skills")))
        r = _run(firm_skill_lazy_loader(
            skills_dir=str(tmp_path / "skills"),
            refresh=True,
        ))
    ...
    def test_nonexistent_dir(self, tmp_path):
        from src.skill_loader import firm_skill_lazy_loader
        r = _run(firm_skill_lazy_loader(skills_dir=str(tmp_path / "nope")))
        assert isinstance(r, dict)
        assert r.get("ok") is False
